...
void NdefManager::nfcWriteTag(const QString &nfcTagText)
{
    ...
    if (nfcTagText.startsWith("Corkboards Autostart")) {
        ...
        qDebug() << "Index: " << payloadPos;
        const int payloadLength = nfcTagText.length() - (payloadPos + payloadText.length());
        qDebug() << "length: " << payloadLength;
        if (payloadLength > 0) {
            qDebug() << "Payload: " << nfcTagText.right(payloadLength);
...
void NdefManager::writeCachedNdefMessage()
{
    if (pendingWriteNdef)
    {
        if (cachedTarget)
        {
            ...
            {
                // Use for old Qt Mobility 1.2 beta target from the April sis file:
